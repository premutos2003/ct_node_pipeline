...
terraform apply --auto-approve -var stack=${STACK} -var kms_key_arn=${kms} -var aws_access_key=${AWS_ACCESS_KEY} -var aws_secret_key=${AWS_SECRET_KEY} -var git_project=${PROJECT_NAME} -var port=${APP_PORT} -var version=${VERSION} -var region=${REGION} '''
    ...
    stage("Build cloud infrastructre") {
        ...
        metadata= $(terraform outpurt-json | jq -r '.')
        app_instance_ip=$(echo $metadata | jq -r '.app_instance_ip.value')
        app_instance_id=$(echo $metadata | jq -r '.app_instance_id')
        app_id=$(echo $metadata | jq -r '.app_id.value')
        stack=$(echo $metadata | jq -r '.stack.value')
        region=$(echo $metadata | jq -r '.region.value')
        curl -X POST -d $metadata 'docker.for.mac.localhost:3000/app_infra'
